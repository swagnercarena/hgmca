	...
	for j in range(j_max+2):
		for ell in range(band_lim):
			if ell < scale_int**(j-1):
				...
				phi2[ell+j*band_lim] = kappa_integral(
					ell/scale_int**j,1.0,n_quads,scale_int)/norm
...
class WaveletsBase():
	""" Base class for conducting the wavlet transforms used by the gmca
	algorithms.
	"""

	def s2dw_wavelet_tranform(self,input_map,output_maps_prefix,band_lim,
		scale_int,j_min,input_fwhm,target_fwhm=None,n_quads=1000,
		...
		# Generate the array that will contain our wavelet maps in healpix
		# space
		...
		if precomputed:
			...
			# Generate the kernel function in harmonic space for our wavelets
			# and scaling functions
			wav_har = np.zeros((j_max+2)*band_lim)
			scale_har = np.zeros(band_lim)
			s2dw_harmonic(wav_har,scale_har,band_lim,scale_int,
				n_quads,j_min)

			# We have another normalizing factor in the tranform (that in this
			# case would have been equivalent to not including a normalizing
			# factor in our original calculation)
			...
			for j in range(j_min,j_max+1):
				...
				wav_bl = (hp.gauss_beam(target_fwhm[j-j_min+1]*a2r,
					lmax=band_lim-1)/hp.gauss_beam(input_fwhm*a2r,
					lmax=band_lim-1))
	...
	def s2dw_wavelet_inverse_transform(self,wavelet_dict,output_fwhm,
		n_quads=1000):
		...
		scale_alm[new_indices] = hp.map2alm(scale_map,
			lmax=scale_lim,datapath=healpy_weights_folder,
		...
		# Initialize our spherical harmonic alm and add them up. Also include
		# the norm
		ell = np.arange(band_lim)
		norm_factor = np.sqrt((4*np.pi)/(2*ell+1))
	...
	def multifrequency_wavelet_maps(self,input_maps_dict,output_maps_prefix,
		scale_int,j_min,precomputed=False,nest=False,
		n_quads=1000):
